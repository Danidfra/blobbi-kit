...
on top of [`@blobbi/core`](../blobbi-core).
...
Depends only on React, TanStack Query, Nostrify, and `@blobbi/core` — never on any
host-app internals.

## Install

```sh
npm install @blobbi/react @blobbi/core react @tanstack/react-query @nostrify/nostrify @nostrify/react
```

All of the above (plus `@blobbi/core`) are peer dependencies.

## Usage

```tsx
import { useProjectedBlobbiState } from '@blobbi/react';

// Deep imports are also supported:
import { useBlobbiInteractions } from '@blobbi/react/hooks/useBlobbiInteractions';
import { computeXp } from '@blobbi/react/lib/blobbi-xp';
```
...
imports resolve 1:1 against `dist/`. `@blobbi/core` is kept external so consumers
